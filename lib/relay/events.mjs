...
export function normalizeExecutorEvent(raw, context = {}, { now = () => new Date().toISOString() } = {}) {
  ...
  const source = raw.source ?? context.source ?? 'unknown';
  const sessionId = raw.session_id ?? context.session_id ?? null;
  const nativeId = raw.event_id ?? raw.eventId ?? raw.id ?? raw.cursor ?? context.source_cursor
    ?? stableId([source, nativeType, JSON.stringify(raw.payload ?? raw.data ?? raw)]);
  const eventId = raw.event_id ?? `E-${stableId([source, sessionId ?? '', String(nativeId)])}`;
  const type = TYPE_MAP[nativeType] ?? nativeType;
  const payload = redactSecrets(raw.payload ?? raw.data ?? {});
  const idempotencyKey = raw.idempotency_key
    ?? `${source}:${sessionId ?? 'no-session'}:${String(nativeId)}`;

  return {
    event_id: eventId,
    workflow_run_id: raw.workflow_run_id ?? context.workflow_run_id,
    task_id: raw.task_id ?? context.task_id ?? null,
    attempt_id: raw.attempt_id ?? context.attempt_id ?? null,
    source,
    type,
    ...
    generation: raw.generation ?? context.generation ?? null
  };
}
