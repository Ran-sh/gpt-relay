...
export class CodexAdapter {
  ...
  constructor({
    ...
    this.#config = { cli, cliArgs: [...cliArgs], environment: { ...environment }, detectTimeoutMs };
  }
  ...
  async #launch(task, context, resumeSessionId) {
    ...
    const sandbox = task.delegated_scope.allowed_changes.length > 0 ? 'workspace-write' : 'read-only';
    const args = [
      ...this.#config.cliArgs,
      'exec',
      '--json',
