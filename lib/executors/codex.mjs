...
const SAFE_PARENT_ENVIRONMENT_KEYS = new Set([
  'PATH', 'PATHEXT', 'SYSTEMROOT', 'WINDIR', 'COMSPEC', 'TEMP', 'TMP', 'TMPDIR',
  'LANG', 'LC_ALL', 'TERM', 'COLORTERM'
]);

export function buildChildEnvironment(task, parentEnvironment = process.env, overrides = {}) {
  const credentialsAllowed = task?.authorization?.credentials === true;
  const environment = credentialsAllowed
    ? { ...parentEnvironment }
    : Object.fromEntries(Object.entries(parentEnvironment).filter(([key]) => SAFE_PARENT_ENVIRONMENT_KEYS.has(key.toUpperCase())));
