...
export class WorkflowDaemon {
  ...
  constructor({
    ...
    this.#resultContractService = resultContractService;
  }
  ...
  async resume(workflowRunId, input = {}, context = {}) {
    ...
    if (input.type === 'approval.granted') {
      ...
      if (attention.status !== 'RESOLVED' || attention.response_type !== 'approval.granted') {
        throw new Error('approval grant requires a resolved approval response');
      }
  ...
  async #execute(task, context = {}, recovery = null) {
    ...
    while (attemptNumber < this.#maxAttempts && actionCount < this.#maxActions) {
      ...
      if (decision.decision === 'COMPLETE') {
        ...
        const resultContract = task.result_contract && this.#resultContractService
          ? this.#resultContractService.write({ task, ...latestResult })
          : null;
        saveWorkflow({
          ...verifyingWorkflow,
          state: 'COMPLETED',
