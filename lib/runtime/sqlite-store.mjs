...
export class SQLiteRuntimeStore {
  ...
  claimJob(jobId, ownerId) {
    requireId(jobId, 'jobId');
    requireId(ownerId, 'ownerId');
    const result = this.#database.prepare(`
      UPDATE runtime_jobs SET status = 'RUNNING', owner_id = ?, attempts = attempts + 1, updated_at = ?
      WHERE job_id = ? AND status = 'PENDING'
    `).run(ownerId, this.#now(), jobId);
  ...
  completeJob(jobId, result = {}) {
    this.#database.prepare(`
      UPDATE runtime_jobs SET status = 'COMPLETED', result_json = ?, owner_id = NULL, updated_at = ?
      WHERE job_id = ? AND status = 'RUNNING'
    `).run(JSON.stringify(result), this.#now(), requireId(jobId, 'jobId'));
    return this.getJob(jobId);
  }

  failJob(jobId, error, { retry = false } = {}) {
    this.#database.prepare(`
      UPDATE runtime_jobs SET status = ?, last_error = ?, owner_id = NULL, updated_at = ?
      WHERE job_id = ? AND status = 'RUNNING'
    `).run(retry ? 'PENDING' : 'FAILED', String(error), this.#now(), requireId(jobId, 'jobId'));
    return this.getJob(jobId);
  }

  requeueRunningJobs(ownerId) {
    const result = this.#database.prepare(`
      UPDATE runtime_jobs SET status = 'PENDING', owner_id = NULL, updated_at = ?
  ...
  claimRemoteRunnerJob(runnerId, expiresAt) {
    const row = this.#database.prepare(`
      SELECT runner_job_id FROM remote_runner_jobs WHERE status = 'PENDING' ORDER BY updated_at LIMIT 1
    `).get();
    if (!row) return null;
    const result = this.#database.prepare(`
      UPDATE remote_runner_jobs SET status = 'LEASED', runner_id = ?, lease_expires_at = ?, updated_at = ?
      WHERE runner_job_id = ? AND status = 'PENDING'
    `).run(requireId(runnerId, 'runnerId'), expiresAt, this.#now(), row.runner_job_id);
