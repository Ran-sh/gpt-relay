...
export class SQLiteRuntimeStore {
  ...
  busyTimeoutMs() {
    const row = this.#database.prepare('PRAGMA busy_timeout').get();
  ...
  heartbeatRemoteRunnerJob(jobId, runnerId, expiresAt, { token, generation, now = this.#now() } = {}) {
    const result = this.#database.prepare(`
      UPDATE remote_runner_jobs SET lease_expires_at = ?, updated_at = ?
      WHERE runner_job_id = ? AND runner_id = ? AND token = ? AND generation = ?
        AND status = 'LEASED' AND lease_expires_at > ?
    `).run(
      expiresAt, this.#now(), requireId(jobId, 'jobId'), requireId(runnerId, 'runnerId'),
      requireId(token, 'token'), generation, now
    );
    return Number(result.changes) === 1;
  }

  completeRemoteRunnerJob(jobId, result, { runner_id, token, generation, now = this.#now() } = {}) {
    const updated = this.#database.prepare(`
      UPDATE remote_runner_jobs SET status = 'COMPLETED', result_json = ?, lease_expires_at = NULL, updated_at = ?
      WHERE runner_job_id = ? AND runner_id = ? AND token = ? AND generation = ?
        AND status = 'LEASED' AND lease_expires_at > ?
    `).run(
      JSON.stringify(result), this.#now(), requireId(jobId, 'jobId'),
      requireId(runner_id, 'runner_id'), requireId(token, 'token'), generation, now
    );
    return Number(updated.changes) === 1 ? this.getRemoteRunnerJob(jobId) : false;
  }
  ...
  listSchedules() {
    return this.#database.prepare('SELECT * FROM schedules ORDER BY schedule_id')
      .all().map((row) => ({
  ...
  upsertSourceConfig(source) {
    const sourceId = requireId(source?.source_id, 'source.source_id');
    const type = requireId(source?.type, 'source.type');
    if (!['file', 'git', 'github'].includes(type)) throw new Error(`unsupported source type: ${type}`);
    const config = source.config ?? {};
    const secretEnv = source.secret_env ?? {};
    const inspect = (value) => {
      if (!value || typeof value !== 'object') return;
      for (const [key, child] of Object.entries(value)) {
        if (/(?:secret|token|password|credential|api[_-]?key)/i.test(key)) {
          throw new Error(`secret ${key} must be referenced through secret_env`);
        }
        inspect(child);
      }
    };
    inspect(config);
    for (const envName of Object.values(secretEnv)) {
      if (typeof envName !== 'string' || !/^[A-Z_][A-Z0-9_]*$/.test(envName)) {
        throw new Error('secret_env values must be environment variable names');
      }
    }
    const existing = this.getSourceConfig(sourceId);
    const revision = (existing?.revision ?? 0) + 1;
    const enabled = source.enabled !== false;
    this.#database.prepare(`
      INSERT INTO source_configs (
        source_id, type, enabled, revision, config_json, secret_env_json, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(source_id) DO UPDATE SET
        type = excluded.type, enabled = excluded.enabled, revision = excluded.revision,
        config_json = excluded.config_json, secret_env_json = excluded.secret_env_json,
        updated_at = excluded.updated_at
    `).run(sourceId, type, enabled ? 1 : 0, revision, JSON.stringify(config), JSON.stringify(secretEnv), this.#now());
  ...
  getSourceConfig(sourceId) {
    return this.#sourceConfigFromRow(this.#database.prepare(
      'SELECT * FROM source_configs WHERE source_id = ?'
    ).get(requireId(sourceId, 'sourceId')));
  }

  listSourceConfigs({ enabledOnly = false } = {}) {
    const sql = enabledOnly
      ? 'SELECT * FROM source_configs WHERE enabled = 1 ORDER BY source_id'
      : 'SELECT * FROM source_configs ORDER BY source_id';
    return this.#database.prepare(sql).all().map((row) => this.#sourceConfigFromRow(row));
  }

  setSourceConfigEnabled(sourceId, enabled) {
    const existing = this.getSourceConfig(sourceId);
    if (!existing) throw new Error(`unknown source config: ${sourceId}`);
    return this.upsertSourceConfig({ ...existing, enabled: Boolean(enabled) });
  }

  saveConfigSnapshot(snapshot) {
    const snapshotId = requireId(snapshot?.snapshot_id, 'snapshot.snapshot_id');
    const scope = requireId(snapshot?.scope, 'snapshot.scope');
    const scopeId = requireId(snapshot?.scope_id, 'snapshot.scope_id');
    if (this.getConfigSnapshot(snapshotId)) throw new Error(`config snapshot already exists: ${snapshotId}`);
    const revision = Number(this.#database.prepare(`
      SELECT COALESCE(MAX(revision), 0) + 1 AS revision
      FROM config_snapshots WHERE scope = ? AND scope_id = ?
    `).get(scope, scopeId).revision);
    const stored = {
      snapshot_id: snapshotId, scope, scope_id: scopeId, revision,
      config: snapshot.config ?? {}, created_at: snapshot.created_at ?? this.#now()
    };
    this.#database.prepare(`
      INSERT INTO config_snapshots (
        snapshot_id, scope, scope_id, revision, config_json, created_at
      ) VALUES (?, ?, ?, ?, ?, ?)
    `).run(snapshotId, scope, scopeId, revision, JSON.stringify(stored.config), stored.created_at);
    return stored;
  }

  #configSnapshotFromRow(row) {
    if (!row) return null;
    return {
      snapshot_id: row.snapshot_id, scope: row.scope, scope_id: row.scope_id,
      revision: row.revision, config: parseJson(row.config_json, {}), created_at: row.created_at
    };
  }

  getConfigSnapshot(snapshotId) {
    return this.#configSnapshotFromRow(this.#database.prepare(
      'SELECT * FROM config_snapshots WHERE snapshot_id = ?'
    ).get(requireId(snapshotId, 'snapshotId')));
  }

  listConfigSnapshots({ scope, scopeId } = {}) {
    if ((scope && !scopeId) || (!scope && scopeId)) throw new Error('scope and scopeId must be provided together');
    const rows = scope
      ? this.#database.prepare(`
          SELECT * FROM config_snapshots WHERE scope = ? AND scope_id = ? ORDER BY revision
        `).all(scope, scopeId)
      : this.#database.prepare('SELECT * FROM config_snapshots ORDER BY created_at, snapshot_id').all();
    return rows.map((row) => this.#configSnapshotFromRow(row));
  }
