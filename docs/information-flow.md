...
An executor permission request stops event collection, cancels the active handle, closes the attempt as `BLOCKED`, and records a redacted Attention. A grant restores only the exact requested authorization in the continuation checkpoint; it does not broaden the original Task Contract.
...
Natural-language fallback is classified conservatively and stored as trace evidence. It never turns a phrase such as “approve” into an executable high-risk action; explicit Attention commands remain the authorization boundary.
